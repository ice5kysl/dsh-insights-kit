...
function readVersion(scopeDir) {
  for (const pkg of ["dsh-web-app", "dsh-base"]) {
    try {
      const parsed = JSON.parse(readFileSync(join(scopeDir, pkg, "package.json"), "utf8"));
...
function seedFromScope(scope) {
  ...
  try {
    ...
    if (asset !== void 0) assetPath = join(assetsDir, asset);
  } catch {
    return null;
  }
...
function hasTests(dir) {
  for (const candidate of ["tests", "test", "__tests__"]) {
    if (existsSync2(join2(dir, candidate))) return true;
  }
  if (existsSync2(join2(dir, "scripts", "smoke.mjs"))) return true;
  try {
...
function hasCi(dir) {
  const workflows = join2(dir, ".github", "workflows");
  try {
...
function readmeBytes(dir) {
  try {
    return statSync2(join2(dir, "README.md")).size;
  } catch {
    return -1;
  }
}
async function runSelfcheck(rawDir, options = {}) {
  ...
  if (!patchRel || !existsSync2(join2(dir, patchRel))) {
  ...
  if (!existsSync2(join2(dir, "lib", "index.js")) || !existsSync2(join2(dir, "lib", "client.js"))) {
  ...
  const clientBundlePath = join2(dir, "lib", "client.js");
  if (existsSync2(clientBundlePath)) {
    const requires = extractRequires(readFileSync2(clientBundlePath, "utf8"));
  ...
  if (!existsSync2(join2(dir, "README.zh-CN.md"))) {
  ...
  if (!existsSync2(join2(dir, "docs"))) {
    drops.push(drop("docs.no-docs-dir", "minor", "\u65E0 docs/ \u76EE\u5F55", "no docs/ directory"));
  }
  if (!existsSync2(join2(dir, "LICENSE")) && !existsSync2(join2(dir, "LICENSE.md"))) {
