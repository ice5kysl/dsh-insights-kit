...
function editBundles(profileDir, name2, op) {
  const file = join2(profileDir, "package.json");
  let manifest;
  try {
  ...
  dsh.profile = profile;
  manifest.dsh = dsh;
  const tmp = join2(profileDir, `package.json.dsh-insights-${process.pid}.tmp`);
  try {
...
async function handleMutation(req, res, store, op) {
  ...
  if (op === "install") {
    ...
    sendJson(res, result2.ok ? 200 : 500, { ok: result2.ok, name: name2, op, status: result2.status, detail: result2.detail, restartRequired: result2.ok });
    return;
  }
  if (!installed) {
  ...
  sendJson(res, code, { ok: result.ok, name: name2, op, status: result.status, detail: result.detail, restartRequired: result.status !== "failed" });
}
