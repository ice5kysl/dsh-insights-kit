...
function resolveObservedLatest(doc, dynamics) {
  ...
  const candidates = [
    doc?.shellDistTags?.latest,
    dynamics?.dsh?.npm?.distTags?.latest
  ];
...
function computeUpgradeCheck(doc, dynamics, installed, current) {
  ...
  const rows = installed.map((name2) => {
    const entry = observed.get(name2.toLowerCase());
    const result = latest !== null ? entry?.results[latest] : void 0;
    const status = result === void 0 ? "unknown" : result.status === "ok" ? "ok" : "fail";
    counts[status] += 1;
    return { name: name2, status };
  });
  return {
...
async function handleRequest(req, res, store, log, getLoader, hotCtx) {
  ...
  try {
    ...
    if (pathname === `${PREFIX}/upgrade-check`) {
      ...
      const installed = inventory.plugins.filter((plugin) => plugin.enabled).map((plugin) => plugin.name);
      sendJson(res, 200, {
        ok: true,
