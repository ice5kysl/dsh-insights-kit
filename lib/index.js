...
function readBundlesShape(profileDir) {
  try {
    const parsed = JSON.parse(readFileSync3(join3(profileDir, "package.json"), "utf8"));
...
async function installPackage(profileDir, name2, env) {
  ...
  if (shape === null || !editBundles(profileDir, name2, "add")) {
...
function wellKnownRoots(env) {
  ...
  if (typeof env.APPDATA === "string" && env.APPDATA !== "") {
    dirs.push(join4(env.APPDATA, "npm", "node_modules", "@deepseek-ai", "dsh"));
  }
  return dirs;
}
...
var DSH_NPM_PACKAGE = "@deepseek-ai/dsh";
var NPM_DIST_TAGS_TTL_MS = 5 * 60 * 1e3;
...
function observedByPkg(doc) {
  const map = /* @__PURE__ */ new Map();
  const plugins = doc?.plugins;
  if (typeof plugins !== "object" || plugins === null || Array.isArray(plugins)) return map;
  for (const [name2, row] of Object.entries(plugins)) {
    if (!name2 || typeof row !== "object" || row === null) continue;
    const resultsRaw = row.results;
    const results = {};
    if (typeof resultsRaw === "object" && resultsRaw !== null && !Array.isArray(resultsRaw)) {
      for (const [shell, entry] of Object.entries(resultsRaw)) {
        if (!shell || typeof entry !== "object" || entry === null) continue;
        const status = entry.status;
        if (typeof status !== "string" || !status) continue;
        const missingRaw = entry.missing;
        const condRaw = entry.conditional;
        results[shell] = {
          status,
          missing: Array.isArray(missingRaw) ? missingRaw.filter((m) => typeof m === "string") : [],
          ...Array.isArray(condRaw) ? { conditional: condRaw.filter((m) => typeof m === "string") } : {}
        };
      }
    }
    const cls = row.verdict?.cls;
    const version = row.version;
    map.set(name2.toLowerCase(), {
...
function observedGeneratedAt(doc) {
  const at = doc?.generatedAt;
  return typeof at === "string" && at ? at : null;
}
function observedAtShell(entry, shell) {
...
function isNewerBase(a, b) {
  const order = compareBaseVersions(a, b);
  return order !== null && order > 0;
}
function resolveObservedLatest(doc, dynamics) {
  const versionsRaw = doc?.allShellVersions;
  const matrix = Array.isArray(versionsRaw) ? versionsRaw.filter((v) => typeof v === "string" && v.length > 0) : [];
  const tags = doc?.shellDistTags;
  let best = null;
  ...
  const fallback = dynamics?.dsh?.npm?.distTags?.latest;
  if (typeof fallback === "string" && fallback && (matrix.length === 0 || matrix.includes(fallback))) {
    return fallback;
  }
  return null;
}
function computeUpgradeCheck(doc, dynamics, installed, current) {
  ...
  const rows = installed.map(({ name: name2, version }) => {
    const entry = observed.get(name2.toLowerCase());
    const result = latest !== null ? entry?.results[latest] : void 0;
    let status;
    let measuredVersion;
    ...
    counts[status] += 1;
    return { name: name2, status, ...measuredVersion !== void 0 ? { measuredVersion } : {} };
  });
  return {
...
function createStore(options = {}) {
  ...
  function docUrl(name2) {
    return baseUrl ? `${baseUrl}/${DOC_FILES[name2]}.json` : DEFAULT_DOC_URLS[name2] ?? `${DEFAULT_BASE_URL}/${DOC_FILES[name2]}.json`;
  }
  ...
  function npmDistTags() {
    ...
    const task = fetchJson(`${npmRegistry}/${DSH_NPM_PACKAGE.replace(/^@/, "%40")}`).then((doc) => {
      const raw = doc?.["dist-tags"];
  ...
  return {
    ...
    npmDistTags,
    status() {
      const names = ["insights", "scenarios", "dynamics", "compat", "enrich", "compatObserved"];
      const out = {};
      for (const name2 of names) {
        const hit = cache.get(name2);
...
function apply(raw) {
  ...
  const store = createStore({
    ...
    npmRegistry: process.env.DSH_INSIGHTS_NPM_REGISTRY || void 0
  });
  try {
...
async function handleRequest(req, res, store, log, getLoader, hotCtx) {
  ...
  const mutationOp = pathname === `${PREFIX}/install` ? "install" : pathname === `${PREFIX}/uninstall` ? "uninstall" : pathname === `${PREFIX}/disable` ? "disable" : null;
  ...
  try {
    ...
    if (pathname === `${PREFIX}/scenarios`) {
      ...
      const scenarios = (doc.scenarios ?? []).map((scenario) => ({
        ...
        plugins: (scenario.plugins ?? []).map((plugin) => {
          ...
          const entry = pkgName ? observed.get(pkgName.toLowerCase()) : void 0;
    ...
    if (pathname === `${PREFIX}/dynamics`) {
      const [doc, live] = await Promise.all([store.dynamics(), store.npmDistTags()]);
      if (live === null || doc.dsh === void 0) {
      ...
      sendJson(res, 200, {
        ...
        dynamics: {
          ...doc,
          dsh: { ...doc.dsh, npm: { ...doc.dsh.npm, distTags: live, distTagsAt: (/* @__PURE__ */ new Date()).toISOString() } }
        }
      });
      return;
    }
    ...
    if (pathname === `${PREFIX}/upgrade-check`) {
      ...
      const installed = inventory.plugins.filter((plugin) => plugin.enabled).map((plugin) => ({ name: plugin.name, version: plugin.version }));
      sendJson(res, 200, {
        ok: true,
...
async function handleMutation(req, res, store, op, getLoader, hotCtx) {
  ...
  const inventory = readInstalledInventory();
  const row = inventory.plugins.find((plugin) => plugin.name.toLowerCase() === name2.toLowerCase());
  const installed = row !== void 0;
  if (op === "install") {
    if (row?.enabled === true) {
      sendJson(res, 200, { ok: true, name: name2, op, status: "done", note: "already-installed", restartRequired: false });
      return;
    }
    if (row !== void 0 && row.version !== null) {
      if (!editBundles(dir, row.name, "add")) {
        sendJson(res, 500, { ok: false, name: name2, op, status: "failed", detail: "failed to edit dsh.profile.bundles", restartRequired: false });
        return;
      }
      const hot2 = await hotMount(hotCtx, dir, row.name);
      sendJson(res, 200, {
        ok: true,
        name: name2,
        op,
        ...
        note: "re-enabled",
        hot: hot2.ok,
        detail: hot2.reason ?? void 0,
        restartRequired: !hot2.ok
      });
      return;
    }
    const data = await store.insights();
  ...
  if (op === "disable") {
    ...
    if (row?.enabled === false) {
      sendJson(res, 200, { ok: true, name: name2, op, status: "done", note: "already-disabled", restartRequired: false });
      return;
    }
    ...
    if (shape !== "list") {
      sendJson(res, 400, {
        ok: false,
        error: wireError("unsupported-profile", shape === "all" ? `profile ${profile} has no dsh.profile.bundles list \u2014 single-plugin disable is not expressible (every dependency loads)` : `profile ${profile} manifest unreadable`)
      });
      return;
    }
    const dependents2 = findDependents(dir, name2);
    if (dependents2.length > 0) {
      sendJson(res, 409, {
        ok: false,
        error: wireError("has-dependents", `${name2} is a dependency of: ${dependents2.join(", ")}`),
        dependents: dependents2
      });
      return;
    }
    const liveOff2 = await hotUnmount(name2) || await disableEntry(getLoader(), name2);
    if (!editBundles(dir, name2, "remove")) {
      sendJson(res, 500, { ok: false, name: name2, op, status: "failed", detail: "failed to edit dsh.profile.bundles", restartRequired: false });
      return;
    }
    sendJson(res, 200, { ok: true, name: name2, op, status: "done", hot: liveOff2, restartRequired: !liveOff2 });
    return;
  }
  if (!installed) {
    sendJson(res, 404, { ok: false, error: wireError("not-installed", `${name2} is not installed in profile ${profile}`) });
