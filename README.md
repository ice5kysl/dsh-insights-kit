...
| `POST /dsh-insights/install` | One-click install into the active profile (`pnpm add` + append to `dsh.profile.bundles`); body `{name}`; corpus-scoped (the name must be a plugin package in insights.json); requires the `x-dsh-insights-kit: mutate` header; `restartRequired: true` on success |
| `POST /dsh-insights/uninstall` | Drop from the bundles load list + `pnpm remove`; body `{name}`; restricted to actually-installed names; same header requirement |
| `/dsh-insights/health` | Liveness + per-document cache age + `mutations` capability flag (kill switch `DSH_INSIGHTS_NO_MUTATE=1` and pnpm probe) |

- **Upstream**: `https://dsh-insights.com/data/{insights,scenarios,dynamics,compat,enrich}.json` — fetched lazily on first request, cached in memory with a **6-hour TTL**; failed fetches return **502 + JSON error** and never poison the cache.
