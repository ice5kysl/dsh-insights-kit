...
| `POST /dsh-insights/uninstall` | Live-disable the loader entry (or dispose the hot mount) + drop from the bundles list + `pnpm remove`; body `{name}`; restricted to actually-installed names; same header requirement |
