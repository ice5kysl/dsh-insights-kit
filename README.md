...
| `/dsh-insights/installed` | The active profile's installed plugins, read from the profile manifest (`~/.dsh/profiles/<profile>`; override via `DSH_INSIGHTS_PROFILE_DIR` / `DSH_INSIGHTS_PROFILE`): `{profile, baseline, plugins: [{name, spec, version, plugin}]}` — in-box `@deepseek-ai/*` bundles counted as `baseline`, not listed; local filesystem only, no upstream fetch |
